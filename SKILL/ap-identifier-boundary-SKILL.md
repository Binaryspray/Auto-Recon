...
- **겹침 허용**: 하나의 AP가 여러 boundary에 걸릴 수 있다
...
### Recon 단서
- **js_endpoints**: `/login`, `/signup`, `/reset-password`, `/verify-email`, `/change-email`
- **js_endpoints**: `/mfa/setup`, `/mfa/verify`, `/backup-codes`, `/recovery`
- **js_endpoints**: `/oauth/authorize`, `/oauth/callback`, `/sso/saml`
- **js_endpoints**: `/api/v1/authn`, `/api/v1/sessions`, `/token/refresh`
- **urls**: password reset URL 패턴 (토큰 길이, 만료 시간 힌트)
- **live_hosts**: Okta, Auth0, Cognito 등 IdP 사용 흔적
...
- **js_endpoints**: POST/PUT/PATCH 엔드포인트 — 상태 변경 기능
- **js_endpoints**: `/search`, `/filter`, `/query` — injection 후보
- **js_endpoints**: 파라미터에 `filename`, `path`, `url` — SSRF/LFI 후보
- **nuclei**: open-redirect, ssrf, sql-injection 패턴

### 이어지는 취약점
- `data.mass_assignment` — 서버가 의도하지 않은 필드까지 반영
- `data.injection` — SQL, NoSQL, LDAP, template injection
