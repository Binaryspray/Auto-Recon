...
async fn main() -> Result<()> {
    ...
    match cli.command {
            ...
            while !pending.is_empty() && round <= max_rounds {
                ...
                for handle in &pending {
                    ...
                    let h = tokio::spawn(async move {
                        ...
                        let policy = client.fetch_program_policy(&handle).await;
                        (handle, scopes, policy)
                    });
                    handles.push(h);
                }
                ...
                for h in handles {
                    let (handle, scopes_result, policy_result) = h.await?;
                    match scopes_result {
                        Ok(scopes) => {
                            cache.upsert_scopes(&handle, &scopes).await?;
                            // Save policy if fetched
                            if let Ok(Some(policy)) = policy_result {
                                let _ = cache.upsert_policy(&handle, &policy).await;
                            }
                            done += 1;
                        }
                        Err(_) => {
        ...
        Commands::Rerun { project_id, projects_dir, force, skip } => {
            let projects_dir = get_projects_dir(projects_dir.as_deref());
            let skip_steps: std::collections::HashSet<String> = skip.into_iter().collect();
            let base = std::path::Path::new(&projects_dir);

            let project_dir = if let Some(pid) = project_id {
                let dir = base.join(&pid);
                if !dir.exists() {
                    // Try fuzzy match — find project dirs starting with the given string
                    let mut matches = Vec::new();
                    if let Ok(entries) = std::fs::read_dir(base) {
                        for entry in entries.flatten() {
                            let name = entry.file_name().to_string_lossy().to_string();
                            if name.starts_with(&pid) && entry.path().is_dir() {
                    ...
                    match matches.len() {
                        0 => {
                            eprintln!("Project not found: {}", pid);
                        ...
                        1 => matches[0].clone(),
                        _ => {
                            eprintln!("Ambiguous project_id '{}'. Matches:", pid);
                            for m in &matches {
                ...
                if let Ok(entries) = std::fs::read_dir(base) {
                    for entry in entries.flatten() {
                        if entry.path().is_dir() && entry.path().join("rule.csv").exists() {
                ...
                if projects.is_empty() {
                    println!("No projects found in {}. Run `h1scout select` first.", projects_dir);
                    return Ok(());
                }

                let selection = dialoguer::Select::new()
                    .with_prompt("Select project to rerun")
                    .items(&projects)
                    .interact()?;

                base.join(&projects[selection])
            };

            println!("Re-running recon: {}", project_dir.file_name().unwrap_or_default().to_string_lossy());
            recon::run_recon(&project_dir, force, &skip_steps).await?;
        }

        Commands::Review { project_id, all } => {
            let projects_dir = get_projects_dir(None);
            review::run_review(&projects_dir, project_id.as_deref(), all).await?;
        }
    }
